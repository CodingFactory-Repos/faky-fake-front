...
import { Upload, ImageIcon, AlertTriangle, CheckCircle, Check } from "lucide-react";
...
import { ImageDetectionService, PredictionResult } from "@/lib/services/imageDetection";
...
export default function Home() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { status, data } = await axios.get(`${process.env.NEXT_PUBLIC_BACK_URL}/healthcheck`);
    ...
    <main className="min-h-screen bg-gradient-to-b from-background to-muted p-8">
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="text-center space-y-4">
          <h1 className="text-4xl font-bold tracking-tight">Fake Image Detector</h1>
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-semibold">
                    {prediction.is_fake ? "AI-Generated Image Detected" : "Authentic Image"}
                  </h3>
                </div>

                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Probability of being fake </span>
                    <span className="font-medium"> {(parseFloat(prediction.fake_probability.toFixed(4)) * 100).toFixed(2)}%</span>
                  </div>
                  <Progress
                    value={prediction.fake_probability}
                    className={cn(
                ...
                <div className="text-sm text-muted-foreground">
                  Analysis performed using <span className="font-medium">{prediction.model_used}</span>
                </div>
              </div>
            )}
